import {TransactionBuilderABI} from 'aptos'
...
export class TransactionCreator{
    ...
    static buildCoinTransferTxnPayload(
        ...
        const txnBuilder = new TransactionBuilderABI(COIN_ABIS.map((abi)=>new HexString(abi).toUint8Array()))
    ...
    static buildTransferPayload(address:string, amount: bigint) : TxnBuilderTypes.TransactionPayload{
        const coin = "0x1::aptos_coin::AptosCoin"
        return TransactionCreator.buildCoinTransferTxnPayload(address, amount, coin);
    }
    ...
    {
        const sequenceNumber = await this.client.getSequenceNumber(sender.address())
        console.log(sequenceNumber);
        const maxGasAmount =  DEFAULT_MAX_GAS_AMOUNT;
        const gasUnitPrice = await this.client.getGasEstimate()
        const expireTimestamp = BigInt(Math.floor(Date.now() / 1000) + DEFAULT_Txn_TIMEOUT)
